...
async def on_message(message):
    ...
    if check is True:
        #print(f'swear found from {message.author}')
        for index, elem in enumerate(x):
            if elem in blackList:
                #print(swear was found at place {index}")
                location.insert(1, index)
            #print(location)
        ...
        await message.channel.send(f"Message from {message.author} filtered into:" )
